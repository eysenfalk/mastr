...
fn spawn_server(config: &Path, runtime: &Path, api: &Path) -> SpawnedHerdr {
    fs::create_dir_all(config.join("mastr")).unwrap();
    fs::create_dir_all(runtime).unwrap();
    register_runtime_dir(runtime);
    fs::write(config.join("mastr/config.toml"), "onboarding = false\n").unwrap();
...
fn same_tab_geometry_follows_meaningful_client_activity() {
    ...
    let api = runtime.join("mastr.sock");
    let clients = runtime.join("mastr-client.sock");
