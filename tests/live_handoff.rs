...
fn spawn_server_with_env(
    ...
    cmd.env(
        ...
        runtime_dir.join("mastr-client.sock"),
    );
...
fn spawn_named_session_server(
    ...
    fs::create_dir_all(config_home.join("mastr-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("mastr-dev/config.toml"),
        "onboarding = false\n",
    )
    .unwrap();
    ...
    cmd.env("XDG_CONFIG_HOME", config_home);
    cmd.env("XDG_RUNTIME_DIR", runtime_dir);
    cmd.env("MASTR_SESSION", session_name);
    cmd.env_remove("MASTR_SOCKET_PATH");
...
fn spawn_server_with_args_and_socket_env(
    ...
    if let Some(api_socket_env) = api_socket_env {
        cmd.env("MASTR_SOCKET_PATH", api_socket_env);
    } else {
        cmd.env_remove("MASTR_SOCKET_PATH");
    }
    if let Some(client_socket_env) = client_socket_env {
        cmd.env("MASTR_CLIENT_SOCKET_PATH", client_socket_env);
    } else {
        cmd.env_remove("MASTR_CLIENT_SOCKET_PATH");
    }
    cmd.env("SHELL", "/bin/sh");
...
fn live_handoff_unknown_pane_exit_preserves_session_on_shutdown() {
    ...
    let session: serde_json::Value = serde_json::from_slice(
        &fs::read(config_home.join("mastr-dev/session.json")).expect("saved session"),
    )
...
fn live_handoff_preserves_named_session_socket_paths() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("mastr-dev/sessions/work");
    let api_socket = session_dir.join("mastr.sock");
    let client_socket = session_dir.join("mastr-client.sock");
    ...
    assert!(
        !config_home.join("mastr-dev/mastr.sock").exists(),
...
fn live_handoff_ignores_leaked_default_socket_env_for_named_session() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let default_session_dir = config_home.join("mastr-dev");
    let default_api_socket = default_session_dir.join("mastr.sock");
    let default_client_socket = default_session_dir.join("mastr-client.sock");
    let work_session_dir = config_home.join("mastr-dev/sessions/work");
    let work_api_socket = work_session_dir.join("mastr.sock");
    let work_client_socket = work_session_dir.join("mastr-client.sock");
...
fn live_handoff_preserves_client_socket_env_without_api_socket_env() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("mastr-dev/mastr.sock");
...
fn live_handoff_preserves_installed_plugins() {
    ...
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("mastr-dev/mastr.sock");
    let registry_path = config_home.join("mastr-dev/plugins.json");
...
fn live_handoff_accepts_canonical_pane_id_from_child_env() {
    ...
    assert_ok(request(
        ...
        serde_json::json!({
            ...
            "method": "pane.send_input",
            "params": {"pane_id": pane_id, "text": format!("printf '%s' \"$MASTR_PANE_ID\" > {}", pane_id_marker.display()), "keys": ["Enter"]}
        }),
    ));
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("mastr-dev/mastr.sock")),
        (
            Some("work"),
            config_home.join("mastr-dev/sessions/work/mastr.sock"),
        ),
    ];
...
fn live_handoff_import_failure_rolls_back_old_server_at(failure_point: &str) {
    ...
    let spawned = spawn_server_with_env(
        ...
        &runtime_dir,
        &api_socket,
        &[("MASTR_TEST_HANDOFF_IMPORT_FAIL", failure_point)],
    );
    wait_for_socket(&api_socket, Duration::from_secs(10));
    register_runtime_dir(&runtime_dir);
