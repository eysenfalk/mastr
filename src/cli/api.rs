...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: mastr api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Herdr API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `mastr api schema --json` to print the full schema.\nUse `mastr api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("mastr api commands:");
    eprintln!("  mastr api snapshot");
    eprintln!("  mastr api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: mastr api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Use `mastr api schema --json`"));
        assert!(text.len() < 400);
    }
}
