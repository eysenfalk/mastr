...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: mastr workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: mastr workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: mastr workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: mastr workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: mastr workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: mastr workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: mastr workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let (raw_workspace_id, close_group) = match args {
        ...
        _ => {
            eprintln!("usage: mastr workspace close <workspace_id> [--group]");
            return Ok(2);
        }
    };
...
fn print_workspace_help() {
    eprintln!("mastr workspace commands:");
    eprintln!("  mastr workspace list");
    eprintln!("  mastr workspace create [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]");
    eprintln!("  mastr workspace get <workspace_id>");
    eprintln!("  mastr workspace focus <workspace_id>");
    eprintln!("  mastr workspace rename <workspace_id> <label>");
    eprintln!("  mastr workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  mastr workspace close <workspace_id> [--group]");
}
