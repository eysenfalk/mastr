...
///
/// Contractual override behavior for auto-detect uses `MASTR_SOCKET_PATH`.
/// This variable is kept as a fallback for callers that explicitly need a
/// client-only override when `MASTR_SOCKET_PATH` is not set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = "MASTR_CLIENT_SOCKET_PATH";
...
/// 2. If `MASTR_SOCKET_PATH` is set, derive the client socket path from it by
///    inserting `-client` before `.sock` (e.g. `herdr.sock` -> `herdr-client.sock`).
///    This keeps JSON API and client socket overrides consistent.
/// 3. Otherwise, honor `MASTR_CLIENT_SOCKET_PATH` (legacy/testing fallback).
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "mastr server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_defaults_to_config_dir() {
        ...
        assert_eq!(path, crate::config::config_dir().join("mastr-client.sock"));
    }
