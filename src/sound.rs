...
const DISABLE_SOUND_ENV: &str = "MASTR_DISABLE_SOUND";
#[cfg(any(windows, test))]
const WINDOWS_SOUND_PATH_ENV: &str = "MASTR_SOUND_PATH";
#[cfg(not(any(windows, target_os = "macos")))]
...
$Path = [Environment]::GetEnvironmentVariable('MASTR_SOUND_PATH', 'Process')
if ([string]::IsNullOrWhiteSpace($Path)) { throw 'MASTR_SOUND_PATH is not set' }
...
mod tests {
    ...
    fn windows_media_player_uses_process_environment_and_dispatcher() {
        ...
        assert!(script.contains("GetEnvironmentVariable('MASTR_SOUND_PATH', 'Process')"));
