...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: mastr terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: mastr terminal session control <target> [--takeover] [--cols N] [--rows N]";
pub(crate) const AGENT_HELP_FOOTER: &str = concat!(
    ...
    "    SKIP if a Herdr skill is already in your context. Otherwise run: mastr --skill",
);
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: mastr channel set <stable|preview>");
        return Ok(2);
    };
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `mastr update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("mastr channel commands:");
    eprintln!("  mastr channel show                  print the configured update channel");
    eprintln!("  mastr channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: mastr config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: mastr config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: mastr config reset-keys");
        return Ok(2);
    }
    ...
    println!("If a Herdr server is running, run `mastr server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: mastr session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: mastr session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: mastr session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: mastr session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: mastr session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: mastr terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: mastr terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: mastr terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: mastr terminal title set <title>");
            eprintln!("       mastr terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: mastr terminal title set <title>");
...
fn print_config_help() {
    eprintln!("mastr config commands:");
    eprintln!("  mastr config check  validate config.toml and print diagnostics");
    eprintln!("  mastr config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("mastr terminal commands:");
    eprintln!("  mastr terminal attach <terminal_id> [--takeover]");
    eprintln!("  mastr terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  mastr terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  mastr terminal title set <title>");
    eprintln!("  mastr terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_session_help() {
    eprintln!("mastr session commands:");
    eprintln!("  mastr session list [--json]");
    eprintln!("  mastr session attach <name>");
    eprintln!("  mastr session stop <name> [--json]");
    eprintln!("  mastr session delete <name> [--json]");
...
mod tests {
    ...
    fn parse_env_assignment_accepts_empty_values() {
        assert_eq!(
            super::parse_env_assignment("MASTR_ROLE=").unwrap(),
            ("MASTR_ROLE".to_string(), String::new())
        );
    }
    ...
    fn parse_env_assignment_requires_key_value_separator() {
        assert_eq!(
            super::parse_env_assignment("MASTR_ROLE").unwrap_err(),
