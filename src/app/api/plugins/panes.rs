...
impl App {
    ...
    fn plugin_pane_launch_env(
        ...
        env.push(("MASTR_ENV".to_string(), "1".to_string()));
        env.push(("MASTR_PLUGIN_ID".to_string(), plugin.plugin_id.clone()));
        env.push((
            "MASTR_PLUGIN_ENTRYPOINT_ID".to_string(),
            entrypoint.to_string(),
        ));
        env.push(("MASTR_PLUGIN_CONTEXT_JSON".to_string(), context_json));
...
fn plugin_pane_protected_env_key(key: &str) -> bool {
    matches!(
        ...
        crate::api::SOCKET_PATH_ENV_VAR
            ...
            | "MASTR_PLUGIN_ID"
            | "MASTR_PLUGIN_ROOT"
            | "MASTR_PLUGIN_CONFIG_DIR"
            | "MASTR_PLUGIN_STATE_DIR"
            | "MASTR_PLUGIN_ENTRYPOINT_ID"
            | "MASTR_PLUGIN_CONTEXT_JSON"
            | "MASTR_BIN_PATH"
    )
}
