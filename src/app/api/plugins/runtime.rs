...
impl App {
    pub(super) fn start_plugin_command(
        ...
        env.extend([
            ...
            ("MASTR_ENV".to_string(), "1".to_string()),
            ("MASTR_PLUGIN_ID".to_string(), plugin.plugin_id.clone()),
            ("MASTR_PLUGIN_CONTEXT_JSON".to_string(), context_json),
        ]);
        ...
        if let Some(action_id) = action_id.as_ref() {
            env.push(("MASTR_PLUGIN_ACTION_ID".to_string(), action_id.clone()));
        }
        if let Some(event) = event.as_ref() {
            env.push(("MASTR_PLUGIN_EVENT".to_string(), event.clone()));
        }
        if let Some(event_json) = event_json {
            env.push(("MASTR_PLUGIN_EVENT_JSON".to_string(), event_json));
        }
        if let Some(workspace_id) = context.workspace_id.as_ref() {
            env.push(("MASTR_WORKSPACE_ID".to_string(), workspace_id.clone()));
        }
        if let Some(tab_id) = context.tab_id.as_ref() {
            env.push(("MASTR_TAB_ID".to_string(), tab_id.clone()));
        }
        if let Some(pane_id) = context.focused_pane_id.as_ref() {
            env.push(("MASTR_PANE_ID".to_string(), pane_id.clone()));
        }
        if let Some(clicked_url) = context.clicked_url.as_ref() {
            env.push(("MASTR_PLUGIN_CLICKED_URL".to_string(), clicked_url.clone()));
        }
        if let Some(link_handler_id) = context.link_handler_id.as_ref() {
            env.push((
                "MASTR_PLUGIN_LINK_HANDLER_ID".to_string(),
