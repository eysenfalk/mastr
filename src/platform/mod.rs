...
pub(crate) fn parse_agent_env_hint(environ: &[u8]) -> Option<crate::detect::Agent> {
    for record in environ.split(|&byte| byte == 0) {
        let Some(value) = record.strip_prefix(b"MASTR_AGENT=") else {
            continue;
        };
...
mod tests {
    ...
    fn parse_agent_env_hint_accepts_known_agents() {
        assert_eq!(
            parse_agent_env_hint(b"PATH=/bin\0MASTR_AGENT=claude\0TERM=xterm\0"),
            Some(crate::detect::Agent::Claude)
        );
        assert_eq!(
            parse_agent_env_hint(b"MASTR_AGENT=codex"),
    ...
    fn parse_agent_env_hint_ignores_missing_or_unknown_agents() {
        ...
        assert_eq!(parse_agent_env_hint(b"MASTR_AGENT=not-an-agent\0"), None);
    }
