...
# MASTR_INTEGRATION_ID=hermes
# MASTR_INTEGRATION_VERSION=5
...
def _pane_id() -> str | None:
    if os.environ.get("MASTR_ENV") != "1":
        return None
    return os.environ.get("MASTR_PANE_ID", "").strip() or None
...
def _send_session(session_id: str, start_source: str) -> None:
    ...
    if pane_id is None:
        return
    command = [
        os.environ.get("MASTR_BIN_PATH") or "herdr",
        "pane",
