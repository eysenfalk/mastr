...
if ($env:MASTR_ENV -ne "1") { Exit-Hook }
if ([string]::IsNullOrWhiteSpace($env:MASTR_PANE_ID)) { Exit-Hook }
...
try {
    & $herdr pane report-agent-session $env:MASTR_PANE_ID --source herdr:cursor --agent cursor --seq $seq --agent-session-id $sessionId 2>$null | Out-Null
} catch {
}
